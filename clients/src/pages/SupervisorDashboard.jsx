...
// Define the SupervisorDashboard component
export default function SupervisorDashboard() {
  ...
  // State to determine the type of submission (own or participant)
  const [userType, setUserType] = useState("own"); // Default to "own"
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status !== 200) {
        ...
        setError(errorData); // Set the error state with the error details
        return; // Exit the function early
      }
        ...
        {seeForm && (
          <form onSubmit={handleSubmit}>
            <div>
              ...
              {/* Dropdown to select the type of submission */}
              <div className="mb-4">
                <label className="block mb-2">Submission Type</label>
                <select
                  onChange={(e) => setUserType(e.target.value)} // Update userType based on selection
                  value={userType} // Set the current value of the dropdown
                  className="border border-gray-300 rounded-md p-2 w-full"
                >
                  <option value="own">Submit Own</option>{" "}
                  {/* Option to submit own form */}
                  <option value="participant">Submit Participant</option>{" "}
                  {/* Option to submit for a participant */}
                </select>
              </div>
              {/* If userType is "participant", show additional fields */}
                  ...
                  <div className="mb-4">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setNewFormData((prev) => ({
                          ...prev,
                          userId: e.target.value, // Update userId in newFormData
                        }))
                      }
                      required
                      className="border border-gray-300 rounded-md p-2 w-full"
                  ...
                  <div className="mb-4">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setNewFormData((prev) => ({
                          ...prev,
                          email: e.target.value, // Update email in newFormData
                        }))
                      }
                      required
                      className="border border-gray-300 rounded-md p-2 w-full"
